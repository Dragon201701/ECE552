...
sub compile_assemble() {
  ...
  my $retval = bin_exists("$bin_root/assemble-test.sh");
  if ( ($retval == 0) && (-r $prog_file) ) {
    my $c0 = "$bin_root/assemble-test.sh $prog_file";
    if ($brief) {
      $c0 = "$c0 >> wsrun.log";
    }
    system($c0);
    ...
    print "Could not find the assembler assemble-test.sh in your PATH\n";
    die "";
  }
}
